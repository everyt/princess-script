...
function parseBlank(keyword, tokens, state) {
    if (isKeyword(keyword, state)) {
        ...
        if (parts.length === 2) {
            ...
            if (sceneName.endsWith(':')) {
                ...
                throwError('Expected colon after scene name.', state);
            }
            ...
            throwError('Expected space after scene keyword.', state);
        }
    }
}

function parseColon(keyword, tokens, state, afterColon) {
    if (isKeyword(keyword, state)) {
        const parts = state.script.split(':');
        
        if (parts.length === 1 && afterColon) {
            tokens.push({
                type: keyword,
                value: ''
            });
        } else if (parts.length === 1 && !afterColon){
...
function loadScript(input, options) {
    let script = linebreaker(input, lines);
    let tokens = [];

    let state = new State(options, script);

    while (state.position.line < input.length) {
        while (state.position.column < script.length) {
            parseBlank('scene', tokens, state);
            parseColon('stage', tokens, state, false);
            for (let keyword of KEYWORDS) {
                parseColon(keyword, tokens, state, true);
            }
        }
        state.line++;
    }
