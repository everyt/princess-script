...
function parseColon(keyword, isAfterColon, tokens, state) {
    if (isKeyword(keyword, state)) {
        const parts = state.script.split(':');
...
function loadScript(input, options) {
    ...
    while (state.position.line < input.length) {
        while (state.position.column < script.length) {
            parseBlank('scene', state, tokens);
            parseColon('stage', false, state, tokens);
            for (let keyword of KEYWORDS) {
                parseColon(keyword, true, state, tokens);
            }
        }
        state.position.line++;
    }
