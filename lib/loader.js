...
function commentProcess(array, state) {
  for (let i = 0; i < array.length; i++) {
    state.line++;

    array[i] = slice(array[i], 0, array[i].indexOf('//'));

    for (let keyword of KEYWORD_COMMENT_ARRAY) {
      const keyword1 = Object.keys(keyword)[0];
      const Keyword2 = keyword[keyword1];

      const prefix = slice(array[i], 0, array[i].indexOf(keyword1));
      const subfix = slice(array[i], array[i].indexOf(Keyword2));

      array[i] = prefix + subfix;
    }
  }
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].hasOwnProperty('keyword')) {
      if (arr[i].keyword === '변수') {
        ...
        type = 'script';
      } else if (arr[i].keyword === '씬' || arr[i].keyword === '끝') {
        // 예외처리 없이 넘어가기
      } else {
...
function PostProcess(AST) {
  ...
  const isFuncKeyword = (keyword) => {
    return keyword === '효과' || keyword === '선택지' || keyword === '종료' || keyword === '변경' || keyword === '점프'
      ? true
      : false;
  };
  ...
  for (const elemental of AST.script) {
    for (let i = 0; i < elemental.length; i++) {
      if (elemental[i].type == 'variable' && AST.var[elemental[i].content]) {
        ...
        elemental[i].type == 'variable' &&
        !AST.var[elemental[i].content] &&
        !isFuncKeyword(elemental[i].content)
      ) {
    ...
    if (elemental.length > 1) {
      ...
      if (!isFuncKeyword(variableName)) variableValue = meltScript(variableValue);
...
function loadScript(file, content, format = 'UTF-8') {
  ...
  let array = divideBlock(content); //KEYWORD_BLOCK을 기준으로 스크립트를 분해합니다.
  array = commentProcess(array, state); //주석을 제거합니다. //형태의 주석과 KEYWORD_COMMENT_ARRAY에 포함된 주석.
  array = tokenProcess(array, state); //레벨1과 레벨2로 나누어 토큰으로 분쇄합니다.
  let AST = ASTProcess(array, state); //AST 구조를 쌓습니다.
  PostProcess(AST, state); //AST를 위한 후처리를 진행합니다.
  return AST;
}
